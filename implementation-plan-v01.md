...
- [ ] BOSS 実機にテスト用デバイス（USB Audio, AirPods 等）が接続可能（BOSS 手動 QA 待ち）
...
- [ ] AC-006: 切断済みデバイス選択でクラッシュしない（BOSS 手動 QA 待ち）
- [ ] §9.1〜9.4 の全 UI 状態を手動で一度ずつ確認（BOSS 手動 QA 待ち）
...
- [ ] USB マイク接続 → メニュー再表示で一覧に出る
- [ ] USB マイク取り外し → 再表示 or Manual Refresh で反映
- [ ] AirPods 接続 → 一覧に出る・切替できる
- [ ] AirPods 切断 → エラーまたは一覧から消える（クラッシュなし）
- [ ] Display Audio（ディスプレイ内蔵）が一覧に出る
...
- [ ] システムデフォルト追従のアプリで入力が変わる
- [ ] Per-App Input Setting 固定のアプリでは変わらない（仕様どおり）
...
| Phase 6 残り（BOSS 手動） | USB/AirPods 抜き差し、AC-006 切断デバイス、Dock 非表示の目視確認 |
